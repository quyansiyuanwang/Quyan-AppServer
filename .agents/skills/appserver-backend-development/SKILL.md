...
5. Prisma schema 改动必须生成迁移；禁止手写、复制或编辑 migration SQL。
6. Controller、DTO、schema 或安全契约变更后执行 `pnpm run openapi:gen:all`，再执行精确测试和 `pnpm --filter @appserver/backend run type-check`。
7. 交付时列出已执行的检查与刻意未执行的高成本检查。
