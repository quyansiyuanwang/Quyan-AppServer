...
export const RELAY_UPSTREAM_FORMATS = ['openai', 'anthropic', 'gemini'] as const
export type RelayUpstreamFormat = (typeof RELAY_UPSTREAM_FORMATS)[number]

export const ALL_RELAY_REQUEST_FORMATS = ['openai-chat-completions', 'anthropic', 'gemini'] as const

/** Complete configured request-format set, including the newer Responses protocol. */
export const RELAY_REQUEST_FORMATS = [
...
export type RelayConfiguredRequestFormat = (typeof RELAY_REQUEST_FORMATS)[number]
export type RelayRequestFormat = 'openai' | RelayConfiguredRequestFormat

/** Formats that can be translated at relay-token scope. Gemini is intentionally excluded. */
export const RELAY_CONVERTIBLE_REQUEST_FORMATS = [
  'openai-chat-completions',
  'openai-responses',
  'anthropic',
] as const
export type RelayConvertibleRequestFormat = (typeof RELAY_CONVERTIBLE_REQUEST_FORMATS)[number]

/** Probe accepts the legacy `openai` alias in addition to all configured formats. */
export const RELAY_PROBE_FORMATS = ['openai', ...RELAY_REQUEST_FORMATS] as const
export type RelayProbeFormat = (typeof RELAY_PROBE_FORMATS)[number]

export const RELAY_REQUEST_FORMAT_ORDER = RELAY_REQUEST_FORMATS
...
export const parseRelayModelNameConstraint = (
...
export const parseRelayRequestFormats = (
  ...
  const result = normalizedFormats
    .split(',')
    .map(normalizeModelEntry)
    .map((item) => (item === 'openai' ? 'openai-chat-completions' : item))
    .filter((item): item is RelayConfiguredRequestFormat =>
      validFormats.has(item as RelayConfiguredRequestFormat),
    )
...
export const supportsRelayRequestFormat = (
  ...
  const normalizedFormats = normalizeModelEntry(allowedFormats).toLowerCase()
  const normalizedRequestFormat =
    requestFormat === 'openai' ? 'openai-chat-completions' : requestFormat
  return parseRelayRequestFormats(normalizedFormats).includes(normalizedRequestFormat)
}

export const isModelNameAllowed = (
  ...
  return (
    !!normalizedModelName &&
    allowedEntries.some((entry) => normalizeModelEntry(entry) === normalizedModelName)
  )
}
...
export const unionUniqueModelIds = (
  ...modelIdLists: Array<ReadonlyArray<string> | null | undefined>
): string[] => {
  const result: string[] = []
  const seen = new Set<string>()
