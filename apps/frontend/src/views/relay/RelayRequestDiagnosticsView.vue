<template>
  <main class="request-diagnostics-page">
    <header class="page-header">
      <div>
        <h1>{{ i18ns.t('relay.requestDiagnostics') }}</h1>
        <p>{{ i18ns.t('relay.requestDiagnosticsDescription') }}</p>
      </div>
    ...
    <section class="toolbar">
      <el-input
        ...
        clearable
        :placeholder="i18ns.t('relay.requestIdSearch')"
        @keyup.enter="search"
      />
      <el-input
        v-model="filters.keyword"
        clearable
        :placeholder="i18ns.t('relay.requestDiagnosticsKeyword')"
        @keyup.enter="search"
      />
      <el-select v-model="filters.outcome" clearable :placeholder="i18ns.t('relay.requestOutcome')">
        <el-option value="success" :label="i18ns.t('relay.requestOutcomeSuccess')" /><el-option
          value="client-error"
          :label="i18ns.t('relay.requestOutcomeClientError')"
        /><el-option value="server-error" :label="i18ns.t('relay.requestOutcomeServerError')" />
      </el-select>
    ...
    <el-table v-loading="loading" :data="records" row-key="requestId" @row-click="open">
      <el-table-column prop="requestId" :label="i18ns.t('balance.requestId')" min-width="260"
      ...
      <el-table-column
        ...
        min-width="140"
      />
      <el-table-column :label="i18ns.t('relay.requestAttempts')" width="100" align="center"
        ><template #default="{ row }">{{ row.attempts.length }}</template></el-table-column
      >
      <el-table-column :label="i18ns.t('relay.requestTime')" min-width="170"
        ><template #default="{ row }">{{ formatTime(row.createTime) }}</template></el-table-column
      >
      <el-table-column :label="i18ns.t('relay.requestActions')" width="90"
        ><template #default="{ row }"
          ><el-button link type="primary" @click.stop="open(row)">{{
            i18ns.t('relay.requestDetails')
          }}</el-button></template
        ></el-table-column
      >
    </el-table>
    ...
    <el-drawer v-model="drawer" :title="selected?.requestId" size="min(760px, 100vw)">
      <el-descriptions v-if="selected" :column="2" border
        ><el-descriptions-item :label="i18ns.t('relay.requestUser')">{{
          selected.username || '-'
        }}</el-descriptions-item
        ><el-descriptions-item :label="i18ns.t('balance.tokenName')">{{
      ...
      <el-table v-if="selected" v-loading="traceLoading" :data="selectedAttempts" class="attempts"
        ><el-table-column
          ...
          prop="statusCode"
          :label="i18ns.t('relay.requestStatusCode')"
          width="100" /><el-table-column
          prop="path"
          :label="i18ns.t('relay.requestPath')"
          min-width="200" /><el-table-column
          prop="totalTokens"
          :label="i18ns.t('balance.totalTokens')"
          width="110"
      /></el-table>
    </el-drawer>
  </main>
...
.toolbar {
  display: grid;
  grid-template-columns: minmax(240px, 1.5fr) minmax(180px, 1fr) 180px auto;
  gap: 10px;
  margin: 16px 0;
}
.attempts {
