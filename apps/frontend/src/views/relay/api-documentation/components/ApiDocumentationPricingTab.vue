...
<template>
  <template v-if="state.pricingTabActivated">
    <el-alert type="warning" :closable="false" style="margin-bottom: 16px">
      <strong>{{ $t('apiDoc.pricingNote') }}</strong>
    </el-alert>

    <div :class="['pricing-toolbar', { 'pricing-toolbar--mobile': !state.isDesktop }]">
      <template v-if="state.isDesktop">
        <el-select
        ...
        <el-popover placement="bottom-start" trigger="click" :width="320">
          ...
          <div class="pricing-more-settings-panel" @click.stop>
            ...
            <div class="pricing-more-settings-item pricing-more-settings-item--between">
              ...
              <el-button class="pricing-unit-toggle" @click="state.toggleTokenPriceUnit">
                {{ tokenPriceUnit }}
              </el-button>
            </div>
        ...
        <el-button @click="state.handleResetFilters">{{ $t('reset') }}</el-button>

        <el-button :icon="Refresh" @click="state.refreshData" :loading="loading" type="primary">
          {{ $t('refresh') }}
        </el-button>
              ...
              >
                ...
                <div class="pricing-inline-control pricing-inline-control--mobile">
                  ...
                  <el-button
                    ...
                    @click="state.toggleTokenPriceUnit"
                  >
                    {{ tokenPriceUnit }}
                  </el-button>
                </div>
    ...
    <ComponentErrorBoundary>
      <ModelPricingTable
        ...
        :normalize-formats="state.normalizeFormats"
        :get-request-model-id="state.getRequestModelId"
        :get-highlight-parts="state.getHighlightParts"
        :get-channels-for-model="state.getChannelsForModel"
        :get-displayed-price-multiplier="state.getDisplayedPriceMultiplier"
        ...
        :on-price-range-change="state.handlePriceRangeChange"
        :on-price-range-reset="state.resetPriceRangeFilter"
        :on-pricing-type-filter-change="state.handlePricingTypeFilterChange"
        :sort-field="sortField"
        :sort-order="sortOrder"
        :on-sort-change="state.handleSortChange"
      />
    </ComponentErrorBoundary>
