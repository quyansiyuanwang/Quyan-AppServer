<template>
  <section class="usage-panel" v-loading="loading">
    <div class="toolbar">
      <div>
        <h2>{{ title }}</h2>
        <p>{{ description }}</p>
    ...
    <el-divider /><el-table :data="logs"
      ...
      ><el-table-column
        prop="chargeAmount"
        :label="t('productResources.charge')"
        width="100"
      ...
      ></el-table
    ><el-empty v-if="!logs.length && !loading" :description="t('productResources.auditEmpty')" />
  </section>
</template>
...
const { t } = i18ns
const props = defineProps<{ product: DeveloperProductCode; title: string; description: string }>()
const loading = ref(false),
  error = ref(''),
  usage = ref<DeveloperProductUsageDto>(),
  logs = ref<DeveloperProductCallLogDto[]>([])
let sequence = 0
const load = async () => {
  ...
  error.value = ''
  try {
    const [nextUsage, nextLogs] = await Promise.all([
      developerProductService.getUsage(props.product),
      developerProductService.listCallLogs(props.product),
    ])
