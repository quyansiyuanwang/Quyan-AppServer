import type { NavigationIcon } from './navigation-icons'
...
export interface NavigationNode<Icon = NavigationIcon> {
  ...
  children?: readonly NavigationNode<Icon>[]
...
const relayMenu = group(
  ...
  [
    ...
    item('relayContentSafetySystem', 'nav.contentSafetySystem', 'Tools', [
...
export const navigationMenuDefinition: readonly NavigationNode[] = [
  ...
  item('consumptionRecords', 'nav.consumptionRecords', 'TrendCharts', [
  ...
  item('oauthClientManagement', 'nav.oauthClientManagement', 'Link', [
  ...
  group('billing', 'nav.financial', 'Wallet', [
    ...
    item('carpoolManagement', 'nav.carpoolManagement', 'CreditCard', [
  ...
  group('content', 'nav.dataServices', 'FolderOpened', [
    item('jsonEndpointManagement', 'nav.jsonEndpoints', 'Document', [
  ...
  group(
    ...
    [
      ...
      item('supportAiConfig', 'nav.supportAiConfig', 'ChatDotRound', [
  ...
  group('system-audit', 'nav.systemAudit', 'Document', [
    ...
    item('dataLifecycle', 'nav.dataLifecycle', 'Document', [
...
export const debugNavigationNode = item('debug', 'nav.debug', 'Operation', [
...
export const isNavigationNodeAllowed = <Icon>(
  node: NavigationNode<Icon>,
...
export const filterNavigationNodes = <Icon>(
  nodes: readonly NavigationNode<Icon>[],
  effectivePermissions: readonly string[],
  isRouteAvailable: (route: RouteName) => boolean,
): NavigationNode<Icon>[] => {
  const visibleNodes: NavigationNode<Icon>[] = []
...
export interface NavigationRouteEntry<Icon = NavigationIcon> {
  node: NavigationNode<Icon>
  parentLabelKeys: readonly string[]
}

export const flattenNavigationRoutes = <Icon>(
  nodes: readonly NavigationNode<Icon>[],
  parentLabelKeys: readonly string[] = [],
): NavigationRouteEntry<Icon>[] =>
  nodes.flatMap((node) => {
...
export const collectVisibleNavigationRoutes = <Icon>(
  nodes: readonly NavigationNode<Icon>[],
  effectivePermissions: readonly string[],
  isRouteAvailable: (route: RouteName) => boolean,
  parentLabelKeys: readonly string[] = [],
