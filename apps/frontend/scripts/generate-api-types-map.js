...
const DESCRIPTORS_OUTPUT_DIR_PATH = path.resolve(__dirname, '../src/client/api-descriptors')
...
function generateApiServices(apiTypes) {
  ...
  return groupApisByController(apiTypes).map(([controllerName, controllerApis]) => {
    const lines = [
      ...
      `} from '../api-descriptors/${toKebabCase(controllerName)}.gen'`,
      '',
      'export {',
      ...controllerApis.map((api) => `  ${api.name},`),
      '}',
...
function generateApiDescriptors(apiTypes) {
  return groupApisByController(apiTypes).map(([controllerName, controllerApis]) => {
    const lines = [
      '// Auto-generated by scripts/generate-api-types-map.js',
      '',
      "import type * as Types from '../types.gen'",
      'import type {',
      ...controllerApis.map((api) => `  ${api.name}ApiType,`),
      "} from '../api-types-map.gen'",
      '',
    ]

    for (const api of controllerApis) {
      const bodyType = api.bodyType || 'never'
      const pathType = api.pathType ? `NonNullable<Types.${api.pathType}>` : 'never'
      const queryType = api.queryType ? `NonNullable<Types.${api.queryType}>` : 'never'
      const responseType =
...
async function main() {
  ...
  console.log('📝 Generating per-controller API descriptors...')
  const descriptorOutputs = generateApiDescriptors(apiTypes)
  fs.rmSync(DESCRIPTORS_OUTPUT_DIR_PATH, { recursive: true, force: true })
  fs.mkdirSync(DESCRIPTORS_OUTPUT_DIR_PATH, { recursive: true })
  for (const descriptorOutput of descriptorOutputs) {
    const outputPath = path.join(DESCRIPTORS_OUTPUT_DIR_PATH, descriptorOutput.fileName)
    fs.writeFileSync(outputPath, descriptorOutput.content, 'utf-8')
