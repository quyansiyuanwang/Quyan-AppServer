...
vi.mock('@/service/modelPricingService', () => ({ modelPricingService: { getModelPricing: getModelPricingMock } }))
vi.mock('@/service/relayChannelService', () => ({ relayChannelService: { listCatalogOptions: listCatalogOptionsMock } }))

const createComposable = () => {
  let scope: EffectScope | null = effectScope()
  let composable: ReturnType<typeof useApiDocumentationPricing> | null = null
  scope.run(() => { composable = useApiDocumentationPricing() })
  if (!composable) throw new Error('Failed to initialize pricing composable')
  return { composable, dispose: () => { scope?.stop(); scope = null } }
}
...
const createPricingRow = (overrides: Partial<PricingModelRow> = {}): PricingModelRow => ({
  id: 'pricing-1', model: 'gpt-4o-mini', modelId: 'openai/gpt-4o-mini', provider: 'openai/gpt-4o-mini',
  pricingType: 'token-based', supportedFormats: 'openai', inputPrice: 1, outputPrice: 2,
  fixedPrice: null, status: 1, createTime: new Date().toISOString(), updateTime: new Date().toISOString(),
  ...overrides,
} as PricingModelRow)
...
describe('useApiDocumentationPricing', () => {
  ...
  it('matches catalog capabilities by model name and request id', () => {
    const { composable, dispose } = createComposable()
    composable.channels.value = [createChannel({ modelCapabilities: [{ catalogModelName: 'gpt-4o-mini', requestModelId: 'provider-id', supportedRequestFormats: ['openai'] }] })]
  ...
  it('uses the catalog fixed multiplier and all context tiers', () => {
    ...
    composable.channels.value = [createChannel({ multiplier: 2, contextLengthMultipliers: [
    ...
    expect(composable.getChannelPriceCell(row, composable.channels.value[0]!)).toMatchObject({
      multiplier: 2, maximumMultiplier: 6, inputPrice: 20, maximumInputPrice: 60,
      outputPrice: 40, maximumOutputPrice: 120,
    })
    dispose()
  })

  it('uses anonymous model-specific price ranges for variable routes', () => {
    const { composable, dispose } = createComposable()
    const row = createPricingRow({ inputPrice: 10, outputPrice: 20 })
    composable.channels.value = [createChannel({ pricingMode: 'range', multiplier: undefined, modelPriceRanges: [{
      catalogModelName: 'gpt-4o-mini', requestModelId: 'openai/gpt-4o-mini', minMultiplier: 0.5, maxMultiplier: 2,
    }], priceMayVary: true })]
    composable.channelPriceMode.value = 'global-lowest'
    expect(composable.getDisplayedPriceMultiplier(row)).toBe(0.5)
    expect(composable.getChannelPriceCell(row, composable.channels.value[0]!)).toMatchObject({
      available: true, multiplier: 0.5, maximumMultiplier: 2, inputPrice: 5, maximumInputPrice: 20,
    })
    dispose()
  })

  it('does not expose topology controls in the catalog composable', () => {
    const { composable, dispose } = createComposable()
