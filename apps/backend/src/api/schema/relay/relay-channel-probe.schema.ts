...
const workflowStepSchema = z.object({
  name: z.string().trim().regex(/^[A-Za-z][A-Za-z0-9_]{0,49}$/),
  ...
  extract: z.record(z.string().trim().regex(/^[A-Za-z][A-Za-z0-9_]{0,49}$/), variablePathSchema).optional(),
...
export const upsertRelayChannelProbeProfileBodySchema = z.object({
  enabled: z.boolean(),
  probeFormat: z.enum(["openai", "anthropic", "gemini"]),
  probeModel: z.string().trim().min(1).max(200),
  probePayload: z.record(z.string(), z.unknown()),
  upstreamCurrency: z.string().trim().toUpperCase().regex(/^[A-Z]{3,12}$/).optional(),
  localCurrency: z.string().trim().toUpperCase().regex(/^[A-Z]{3,12}$/).optional(),
...
export const applyRelayChannelProbeRunsBodySchema = z.object({
  runIds: z.array(z.string().trim().min(1)).min(1).max(100).refine((ids) => new Set(ids).size === ids.length),
});
