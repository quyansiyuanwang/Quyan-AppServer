...
pub async fn handle_relay_command(command: super::super::RelayCommand, api: &ApiClient, json_output: bool) -> Result<()> {
    let value = match command {
        super::super::RelayCommand::Token { command } => match command {
            ...
            super::super::RelayTokenCommand::Update { id } => relay::update_token(api, &id, read_json_stdin()?).await?,
        ...
        super::super::RelayCommand::Channels { command: super::super::ChannelsCommand::List } => relay::channels(api).await?,
    };
    super::common::print_value(value, json_output)
}

pub async fn handle_product_command(command: super::super::ProductCommand, api: &ApiClient, json_output: bool) -> Result<()> {
    let value = match command {
        super::super::ProductCommand::JsonEndpoints { command } => match command {
            super::super::ProductJsonCommand::Get => product::get(api).await?,
            super::super::ProductJsonCommand::Update { file } => product::update(api, serde_json::from_slice(&std::fs::read(file)?)?).await?,
