...
EdgeOne 等仓外构建平台不会随 Git 仓库同步控制台里的构建设置。推荐使用以下配置：

- 框架预设：`Other`
- 根目录：`./`
- 安装命令：`pnpm install --frozen-lockfile --prod=false`
- 编译命令：`pnpm run build:frontend`
- 输出目录：`apps/frontend/dist`

仓库根目录的 `edgeone.json` 声明 SPA fallback；EdgeOne Makers 会在没有匹配静态资源或函数时返回同一个 `index.html`，从而让 `/home`、`/auth/verify` 等深链直接打开并刷新时保持 200。
...
本地前端仍通过 Vite 的 `/api` 代理访问 `http://localhost:10001`。多域名模式使用域级 Cookie（`Domain=.<LOCAL_ROOT_DOMAIN>`）以便中心登录跨子站回落；免特权模式使用主机级 Cookie，因此不能用于验证跨源部署行为，需要验证时应使用独立的本地 HTTPS 反向代理，而不是把 API 域名指向 Vite。
...
# 登录密码传输加密（生产必须提供匹配的 3072+ 位密钥对，不得复用 JWT/Auth Center 密钥）
PASSWORD_ENCRYPTION_PRIVATE_KEY=<PKCS8 PEM>
PASSWORD_ENCRYPTION_PUBLIC_KEY=<SPKI PEM>
PASSWORD_ENCRYPTION_KEY_ID=<可选；留空按公钥指纹生成>
