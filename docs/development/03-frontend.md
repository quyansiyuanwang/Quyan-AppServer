...
- 错误消息统一使用 `getErrorMessage`；规范后端业务响应的本地化 `message` 优先，网络/超时/HTTP 错误使用前端兜底。不要直接展示未知 `Error.message`、HTML 响应或异常对象。
- 显式面向用户的客户端业务异常使用 `createUserFacingError`，文案来自 locale。通知使用 `showRequestErrorNotice(error, localizedFallback)`，不要在全局通知后再通过 `ElMessage` 弹出同一异常；表单内反馈仍可使用 `getErrorMessage`。
- 请求的 `errorPresentation` 为 `global`（默认）、`local` 或 `silent`，只保存在 Axios 配置，不发送到后端。认证恢复和取消请求保留原有控制流；网络字符串错误码不替换成业务数字码。
- `/repair.html` 是独立 Vite HTML 入口，不初始化正常应用、认证 Store 或业务数据库。登录页跳转此入口会卸载当前页面，避免响应式状态回写正在清理的数据。
- 清理范围由现有存储键、作用域、数据库名称和 Cookie 元数据推导。新增应用持久化状态时必须更新所属模块的规范定义与清理测试，不能创建第二份名称映射。
- 修复接口 `POST /v1/auth/browser-state/reset` 要求精确的配置 Origin、JSON 确认体和 `X-Browser-State-Reset: 1`，不依赖旧 JWT/重放会话；以独立命名限流策略保护。只清除当前浏览器凭据，不提供任意账户、Cookie 名称或域的操作能力。
- 部署需真实提供 `repair.html`，不得回退到 `index.html`。后端不可达、存储受限、数据库被占用或无法枚举时报告部分完成，不承诺已清理其他 origin。
